...
const { getAllAlerts } = require("../integrations/alerts");
...
const updateDownloadRequestStatus = async (req, res, next) => {
  try {
    ...
    if (status === "Aceptada") {
      ...
      let data, headers;
      if (downloadRequest.downloadType !== "Alertas") {
        data = await getAllClimateData(
          req.header("Authorization"),
          2000,
          filters
        );
        ...
        data = await getAllAlerts(req.header("Authorization"), 2000, filters);
        headers = [
          { fieldName: "title", text: "Titulo" },
          { fieldName: "description", text: "Descripción" },
          { fieldName: "type", text: "Tipo" },
          { fieldName: "severity", text: "Severidad" },
          { fieldName: "createdAt", text: "Fecha" },
          { fieldName: "resolved", text: "Resuelta" },
          { fieldName: "appliedActions", text: "Medidas tomadas" },
        ];
      }
      ...
      if (researcherDB) {
        ...
        const mailOptions = {
          ...
          <div style="background-color: #ffffff; border-radius: 8px; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1); padding: 20px; max-width: 600px; margin: 0 auto;">
            ...
            <p style="color: #333333; font-size: 16px;">¡Buenas noticias! Su solicitud de descarga de datos ${
              downloadRequest.downloadType === "Alertas"
                ? "de fallas en los nodos"
                : " ambientales"
            } ha sido revisada y aprobada por un administrador. A continuación, encontrará los enlaces para descargar los archivos generados:</p>
